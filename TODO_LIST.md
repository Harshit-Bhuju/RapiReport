...
| **Diet & Nutrition Logging**            | ✅     | Meals logged; DB: `diet_logs`. AI diet suggestions + medicine–diet interaction checks via `ai_diet_suggestion.php` (Gemini).                                                                  |
...
- `health/doctor_patients.php` – GET list of patients (from consultations)
- `health/doctor_patient_timeline.php` – GET timeline (prescriptions + symptoms) + missed doses for a patient
- `health/prescription_stats.php` – GET stats (total Rx, last 30 days, most prescribed meds)
- `api/ai_diet_suggestion.php` – POST mealType + currentMeds + conditions → AI suggestion + interaction warnings
...
- **Prescription Scan** – “Check history only”, clarity score, warnings, alternatives.
...
1. ✅ Doctor dashboard wired to **real patients** from `consultations` table.
2. ✅ **Prescription intel stats** added (total Rx, last 30 days, most prescribed meds).
3. ✅ **Early warning** per patient (missed doses last 7 days) shown on doctor dashboard.
4. ✅ **Diet**: AI suggestion endpoint + medicine–diet interaction check (Gemini).

## Suggested Next Steps

1. **Community**: anonymized aggregation API + heatmap UI.
2. **Doctor review action** on async consult (e.g. mark reviewed, add doctor_notes).
3. **Predictive scheduling** estimates consultation time per patient.
